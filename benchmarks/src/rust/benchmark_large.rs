/*
 * Large Matrix Benchmark - Optimized implementations only
 * Skips slow implementations for large matrices
 */
...
impl LargeBenchmark {
    ...
    unsafe fn ultimate_optimized_matmul(&self, a: &Vec<Vec<f64>>, b: &Vec<Vec<f64>>) -> Vec<Vec<f64>> {
        // Ultimate optimization: Flat arrays + SIMD + Cache blocking
        let mut a_flat = vec![0.0; self.matrix_size * self.matrix_size];
        let mut b_flat = vec![0.0; self.matrix_size * self.matrix_size];
        let mut result_flat = vec![0.0; self.matrix_size * self.matrix_size];
        ...
        F: Fn(&Vec<Vec<f64>>, &Vec<Vec<f64>>) -> Vec<Vec<f64>>,
    {
        ...
        // Print results
        println!("{}:", name);
        println!("  Mean time: {:.3} s ± {:.3} s", mean_time, std_dev);
    ...
    fn run_benchmarks(&self) {
        println!("================================");
        println!("Large Matrix Rust Benchmark");
        ...
        let result = self.flat_array_matmul(&a, &b);
...
fn main() {
    ...
    let benchmark = LargeBenchmark::new(matrix_size, num_runs, 1);
